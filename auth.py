...
from api import request, jsonify, g
...
def route_permission_for_path(path: str, method: str | None = None) -> str | None:
    ...
    if safe_path.startswith("/api/loans/") and (safe_path.endswith("/statement.pdf") or safe_path.endswith("/statement")):
        return "loans.export"
    if safe_path.startswith("/api/loans/") and (
        "/guarantors" in safe_path
        or safe_path.endswith("/restructure")
        or safe_path.endswith("/write-off")
    ):
        return "loans.edit"
